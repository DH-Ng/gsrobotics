...
On this page, you will find
 - Basic [Installation Instructions](#installation-instructions)
...
> [!Note]
> Changes from this repository compared to the [original one](https://github.com/gelsightinc/gsrobotics):
> - changed directory structure 
> - code can be installed as a pip package now (module is called `gelsight`)
...
```
git clone https://github.com/DH-Ng/gsrobotics.git
```
...
2. Run
```sh
python examples/demo_liveview.py
```
...
1. Run `python examples/demo_liveview.py`. You should see a window app for live view session. On top bar you have device selection option. This is especially helpful if you have web cameras, or more than one GelSight Mini. Once a device is choosen, live feed should start.
<table>
<tr><td>
...
3. Select **Save Data** to store current sequence. Each row of data (except for the first description row)
...
1. Run `python examples/demo_view3D.py`. You should se a window app for live view session. By default you should see two windows. One should consist
...
1. Run `python example/opencv_liveview-demo.py`. 
...
```sh
python examples/demo_liveview.py --gs-config default_config.json
```
